...
Ported from the ``dev-chinmay/flowmatch`` branch and extended with the small
adapter surface that ``nav_policy.rl.stochastic_policy.StochasticVelocityPolicy``
expects from a ``base`` model, so SAC can be fine-tuned directly on top of a
flow-matching checkpoint:

    * ``forward_latent(rgb, goal)``   -> [B, gru_hidden + goal_emb_dim]  (== encode)
    * ``predict_mean_first(rgb, goal)`` -> [B, cmd_dim]  (DIFFERENTIABLE flow readout
                                          from x0=0, first horizon step, z-scored)
    * attrs ``cmd_dim`` / ``gru_hidden`` / ``goal_emb_dim`` / ``T`` /
      ``goal_input_dim`` / ``use_depth=False``
...
Conditional vector field (the FM-specific head):
    ...
    Output: H*cmd_dim (velocity field), optional learned skip from z_t.
"""
...
class FlowMatchingPolicy(nn.Module):
    ...
    names and parameter shapes as RGBVelocityPolicy so the encoder state-dict keys
    are interchangeable.
    """
    ...
    def __init__(self,
                 ...
                 vf_use_skip: bool = True,
                 n_val_steps: int = 4) -> None:
        super().__init__()
        ...
        # Euler steps for the deterministic flow readout used as the SAC action mean.
        self.n_val_steps = int(n_val_steps)
        # SAC adapter flag: FM policy is RGB-only (no depth branch).
    ...
    # ── SAC adapter surface (StochasticVelocityPolicy `base` interface) ──────

    def forward_latent(self,
                       rgb_seq: torch.Tensor,
                       goal: torch.Tensor) -> torch.Tensor:
        """Critic latent — identical to RGBVelocityPolicy.forward_latent."""
        return self.encode(rgb_seq, goal)

    def predict_mean_first(self,
                           rgb_seq: torch.Tensor,
                           goal: torch.Tensor,
                           n_steps: Optional[int] = None) -> torch.Tensor:
        """Deterministic flow readout for the first horizon step [B, cmd_dim] (z-space).

        Integrates the vector field with Euler from the zero prior (x0 = 0) so the
        readout is deterministic AND differentiable — the SAC actor gradient flows
        through the flow into the encoder + vector field. (``sample()`` below is the
        stochastic, no-grad variant used for plain inference.)
        """
        context = self.encode(rgb_seq, goal)
        B = rgb_seq.shape[0]
        n = int(n_steps if n_steps is not None else self.n_val_steps)
        x = torch.zeros(B, self.action_dim, device=rgb_seq.device, dtype=rgb_seq.dtype)
        dt = 1.0 / max(n, 1)
        for i in range(n):
            t = torch.full((B,), i * dt, device=x.device, dtype=x.dtype)
            x = x + dt * self.vector_field(x, t, context)
        return x.reshape(B, self.H, self.cmd_dim)[:, 0, :]
    ...
    def sample(self,
        ...
        Returns: commands [B, H, cmd_dim] float32, z-scored.
        """
        B = rgb_seq.shape[0]
        context = self.encode(rgb_seq, goal)
